...
use Hsyngkby\Support\Application as ApplicationContract;
...
class ProviderRepository
{
    ...
    public function load(array $providers)
    {
        ...
        if ($this->shouldRecompile($manifest, $providers) || $this->app['env'] !== 'production') {
    ...
    {
        __l('[ProviderRepository] registerLoadEvents : ' . $provider . ' ' . count($events) . ' Events');

        if (count($events) < 1) return;

        $app = $this->app;

        $app->make('eventsManager')->attach($events, function () use ($app, $provider) {
    ...
    {
        __l('[ProviderRepository] compileManifest');
